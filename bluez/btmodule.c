...
int
hci_send_acl(int dd, uint16_t handle, uint8_t pb_flag, uint8_t bc_flag,
        uint16_t dlen, void *data)
{
...
init_bluetooth(void)
{
	Py_TYPE(&filter_type) = &PyType_Type;
...
// Initialization steps for _bluetooth.
	...
	Py_INCREF((PyObject *)&filter_type);
	if (PyModule_AddObject(m, "hcifilter",
			       (PyObject *)&filter_type) != 0)
		return;
